...
async function main() {
  try {
    ...
    const [authorsAndMentors] = await connection.query(`
  SELECT a.author_name, s.supervisor_name
  FROM authors a
  LEFT JOIN supervisors s ON a.supervisor = s.supervisor_id;
`);

    const [authorsWithPapers] = await connection.query(`
  SELECT a.*, rp.paper_title
  FROM authors a
  LEFT JOIN authors_research_Papers arp ON a.author_id = arp.author_id
  LEFT JOIN research_Papers rp ON arp.paper_id = rp.paper_id;
`);

    const [authorsWithoutPapers] = await connection.query(`
  SELECT a.*
  FROM authors a
  LEFT JOIN authors_research_Papers arp ON a.author_id = arp.author_id
  WHERE arp.paper_id IS NULL;
`);
